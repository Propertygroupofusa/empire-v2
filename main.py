...
async def run_migrations():
    ...
    # TEMPORARILY DISABLED: Auto-increment repairs causing startup crash.
    # The per-table sequence repair below (clients.id in particular) was
    # leaving the transaction aborted ("current transaction is aborted,
    # commands ignored until end of transaction block") and crashing the
    # entire startup. Disabling the whole sweep - including the column
    # add/enum-conversion logic that shared its transaction - to unblock
    # the platform. Bots, dashboards, and trading all function without
    # auto-increment on these tables; only inserts that rely on the
    # database generating an id would be affected, and none of the
    # current code paths do that.
    #
    # async with engine.begin() as conn:
    ...
    log.info(f"Migrations skipped (auto-increment repairs disabled)")
